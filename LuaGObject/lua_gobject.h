...
#define G_LOG_DOMAIN "LuaGObject"
...
// For Lua 5.3
typedef lua_Unsigned lua_gobject_Unsigned;
#define LUA_GOBJECT_LUAINT_FORMAT LUA_INTEGER_FMT
#elif LUA_VERSION_NUM == 502
// For Lua 5.2
typedef lua_Unsigned lua_gobject_Unsigned;
#define lua_isinteger(L, i) 0
#define LUA_GOBJECT_LUAINT_FORMAT "%td"
#else
// For Lua 5.1
typedef unsigned long lua_gobject_Unsigned;
#define lua_isinteger(L, i) 0
#define LUA_GOBJECT_LUAINT_FORMAT "%td"
#endif
...
#define lua_gobject_makeabs(L, x) do { if (x < 0) x += lua_gettop (L) + 1; } while (0)

/* Puts parts of the name to the stack, to be concatenated by lua_concat.
   Returns number of pushed elements. */
int lua_gobject_type_get_name (lua_State *L, GIBaseInfo *info);
...
/* Stores repo type table associated with specified gtype (or BaseInfo
   ...
   found in the repo. */
void lua_gobject_type_get_repotype (lua_State *L, GType gtype, GIBaseInfo *info);

/* Gets GType from Lua index narg.  Accepts number and when it is
   other type, invokes Lua helper to convert. */
GType lua_gobject_type_get_gtype (lua_State *L, int narg);

/* Allocates guard, a pointer-size userdata with associated destroy
   handler. Returns pointer to user_data stored inside guard. */
gpointer *lua_gobject_guard_create (lua_State *L, GDestroyNotify destroy);

/* Creates cache table (optionally with given table __mode), stores it
   into registry to specified userdata address. */
void
lua_gobject_cache_create (lua_State *L, gpointer key, const char *mode);

/* Initialization of modules. */
void lua_gobject_marshal_init (lua_State *L);
void lua_gobject_record_init (lua_State *L);
void lua_gobject_object_init (lua_State *L);
void lua_gobject_callable_init (lua_State *L);
void lua_gobject_gi_init (lua_State *L);
void lua_gobject_buffer_init (lua_State *L);

/* Checks whether given argument is of specified udata - similar to
   luaL_testudata, which is missing in Lua 5.1 */
void *
lua_gobject_udata_test (lua_State *L, int narg, const char *name);

/* Metatable name of userdata for 'bytes' extension; see
   http://permalink.gmane.org/gmane.comp.lang.lua.general/79288 */
#define LUA_GOBJECT_BYTES_BUFFER "bytes.bytearray"

/* Metatable name of userdata - gi wrapped 'GIBaseInfo*' */
#define LUA_GOBJECT_GI_INFO "lua_gobject.gi.info"

/* Creates new instance of info from given GIBaseInfo pointer. */
int lua_gobject_gi_info_new (lua_State *L, GIBaseInfo *info);

/* Assumes that 'typetable' can hold field 'name' which contains
   wrapped LUA_GOBJECT_GI_INFO of function.  Returns address of this function,
...
/* Retrieve synchronization state, which can be used for entering and
   leaving the state using lua_gobject_state_enter() and lua_gobject_state_leave(). */
...
/* Another special value for 'parent' argument, meaning that the value
   ...
   requirements. */
#define LUA_GOBJECT_PARENT_IS_RETVAL (G_MAXINT - 1)
...
#define LUA_GOBJECT_PARENT_CALLER_ALLOC (G_MAXINT - 2)
...
/* Marshalls single value from Lua to GLib/C. Returns number of temporary
   ...
   returns. */
int lua_gobject_marshal_2c (lua_State *L, GITypeInfo *ti, GIArgInfo *ai,
...
gboolean lua_gobject_marshal_2c_caller_alloc (lua_State *L, GITypeInfo *ti,
...
/* Marshalls single value from GLib/C to Lua. If parent is non-0, it
   ...
   resides. */
void lua_gobject_marshal_2lua (lua_State *L, GITypeInfo *ti, GIArgInfo *ai,
...
/* Allocates n-th closure in the closure block for specified Lua
   ...
   closure. */
gpointer lua_gobject_closure_create (lua_State* L, gpointer user_data,
			     int target, gboolean autodestroy);

/* GDestroyNotify-compatible callback for destroying closure. */
void lua_gobject_closure_destroy (gpointer user_data);
...
gpointer lua_gobject_record_new (lua_State *L, int count, gboolean alloc);
...
void lua_gobject_record_2lua (lua_State *L, gpointer addr, gboolean own, int parent);
...
void lua_gobject_record_2c (lua_State *L, gint narg, gpointer target, gboolean by_value,
...
int lua_gobject_object_2lua (lua_State *L, gpointer obj, gboolean own,
		     gboolean no_sink);
...
/* Gets pointer to C-side object represented by given Lua proxy. If
   ...
   requested type. */
gpointer lua_gobject_object_2c (lua_State *L, int narg, GType gtype, gboolean optional,
			gboolean nothrow, gboolean transfer);

#if !GLIB_CHECK_VERSION(2, 30, 0)
/* Workaround for broken g_struct_info_get_size() for GValue, see
   https://bugzilla.gnome.org/show_bug.cgi?id=657040 */
gsize lua_gobject_struct_info_get_size (GIStructInfo *info);
#define g_struct_info_get_size lua_gobject_struct_info_get_size
int lua_gobject_field_info_get_offset (GIFieldInfo *info);
#define g_field_info_get_offset lua_gobject_field_info_get_offset
#endif
...
gpointer lua_gobject_object_get_function_ptr (GIObjectInfo *info,
				      const gchar *(*getter)(GIObjectInfo *));

GIRepository *lua_gobject_gi_get_repository (void);
