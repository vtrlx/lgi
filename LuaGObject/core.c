  ...
  for (i = 1; i <= top; i++)
	...
	case LUA_TNUMBER:
	  if (lua_isinteger(L, i))
		item = g_strdup_printf (LUA_GOBJECT_LUAINT_FORMAT, lua_tointeger (L, i));
	  else
...
int
lua_gobject_type_get_name (lua_State *L, GIBaseInfo *info)
{
  GSList *list = NULL, *i;
  int n = 1;
...
void
lua_gobject_type_get_repotype (lua_State *L, GType gtype, GIBaseInfo *info)
{
  luaL_checkstack (L, 4, "");
  ...
  if (lua_isnil (L, -1))
	...
	{
	  info = gi_repository_find_by_gtype (lua_gobject_gi_get_repository (), gtype);
...
GType
lua_gobject_type_get_gtype (lua_State *L, int narg)
{
  ...
  switch (lua_type (L, narg))
	...
	lua_rawget (L, narg);
	gtype = lua_gobject_type_get_gtype (L, -1);
	lua_pop (L, 1);
	return gtype;
      }
...
#define UD_GUARD "lua_gobject.guard"
...
gpointer *
lua_gobject_guard_create (lua_State *L, GDestroyNotify destroy)
{
...
{
  lua_pushlightuserdata (L, (gpointer) lua_gobject_type_get_gtype (L, 1));
  return 1;
}
...
core_repotype (lua_State *L)
{
  ...
  GIBaseInfo **info = lua_gobject_udata_test (L, 1, LUA_GOBJECT_GI_INFO);
  if (!info)
    gtype = lua_gobject_type_get_gtype (L, 1);
  lua_gobject_type_get_repotype (L, gtype, info ? *info : NULL);
  return 1;
}
...
core_constant (lua_State *L)
{
  ...
  GIArgument val;
  GIConstantInfo *ci = *(GIConstantInfo **) luaL_checkudata (L, 1, LUA_GOBJECT_GI_INFO);
  ...
  lua_gobject_marshal_2lua (L, ti, NULL, GI_DIRECTION_IN, GI_TRANSFER_NOTHING, &val,
		    0, NULL, NULL);
  return 1;
}
...
{
  lua_pushinteger (L, (lua_gobject_Unsigned)luaL_checkinteger (L, 1)
		  & (lua_gobject_Unsigned)luaL_checkinteger (L, 2));
  return 1;
}
...
#define UD_MODULE "lua_gobject.core.module"
...
static const struct luaL_Reg lua_gobject_reg[] = {
  { "log",  core_log },
  { "gtype", core_gtype },
...
luaopen_LuaGObject_lua_gobject_core (lua_State* L)
{
  ...
  /* Try to make itself resident.  This is needed because this dynamic
     ...
     lua_States and loading lua_gobject into them, we try to make the whole
  ...
  /* Register 'lua_gobject.core' interface. */
  lua_newtable (L);
  luaL_register (L, NULL, lua_gobject_reg);
  ...
  /* Initialize modules. */
  lua_gobject_buffer_init (L);
  lua_gobject_gi_init (L);
  lua_gobject_marshal_init (L);
  lua_gobject_record_init (L);
  lua_gobject_object_init (L);
  lua_gobject_callable_init (L);
