...
gpointer
lua_gobject_record_new (lua_State *L, int count, gboolean alloc)
{
  Record *record;
  size_t size;
  ...
  for (;;)
      ...
      else
	{
	  ...
	  void (*free_func)(gpointer) =
	    lua_gobject_gi_load_function (L, -1, "_free");
	  if (free_func)
	    {
...
void
lua_gobject_record_2lua (lua_State *L, gpointer addr, gboolean own, int parent)
{
  Record *record;
  ...
  if (parent == LUA_GOBJECT_PARENT_IS_RETVAL || parent == LUA_GOBJECT_PARENT_FORCE_POINTER)
    parent = 0;
  else
  ...
  if (parent != 0)
      ...
      if (!own)
	{
	  ...
	  void (*refsink_func)(gpointer) =
	    lua_gobject_gi_load_function (L, -4, "_refsink");
	  if (refsink_func)
	    {
	      refsink_func(addr);
	      own = TRUE;
...
record_error (lua_State *L, int narg, const gchar *expected_name)
{
  ...
  lua_pushfstring (L, "%s expected, got %s",
		   expected_name ? expected_name : "lua_gobject.record",
		   lua_tostring (L, -1));
...
void
lua_gobject_record_2c (lua_State *L, int narg, gpointer target, gboolean by_value,
  ...
  if (G_LIKELY (!by_value))
	  ...
	  if (G_LIKELY (record->store == RECORD_STORE_ALLOCATED))
	    {
		 ...
		 function. */
	      void (*refsink_func)(gpointer) =
		lua_gobject_gi_load_function (L, narg, "_refsink");
	      if (refsink_func)
      ...
      if (record)
	{
	  ...
	  void (*copy_func)(gpointer, gpointer) =
	    lua_gobject_gi_load_function (L, -1, "_copy");
	  if (copy_func)
...
record_gc (lua_State *L)
{
  ...
  if (record->store == RECORD_STORE_EMBEDDED
      || record->store == RECORD_STORE_NESTED)
    {
	 ...
	 invoke it if yes. */
      lua_getfenv (L, 1);
      void (*uninit)(gpointer) = lua_gobject_gi_load_function (L, -1, "_uninit");
      if (uninit != NULL)
	uninit (record->addr);
    }
  else if (record->store == RECORD_STORE_ALLOCATED)
...
record_tostring (lua_State *L)
{
  ...
  if (lua_isnil (L, -1))
    {
      lua_pop (L, 1);
      lua_pushfstring (L, "lua_gobject.rec %p:", record->addr);
  ...
  if (lua_isnoneornil (L, 2))
    {
      ...
      lua_pushvalue (L, 1);
      lua_gobject_record_new (L, luaL_optinteger (L, 3, 1), alloc);
    }
  else
    {
	...
	: (gpointer) luaL_checkinteger (L, 2);
      ...
      lua_pushvalue (L, 1);
      lua_gobject_record_2lua (L, addr, own, 0);
    }
...
record_query (lua_State *L)
{
  ...
  if (mode < 2)
      ...
      else
	{
	  gpointer addr;
	  lua_pushvalue (L, 3);
	  lua_gobject_record_2c (L, 1, &addr, FALSE, FALSE, TRUE, FALSE);
	  lua_pushlightuserdata (L, addr);
	}
...
record_field (lua_State *L)
{
  ...
  lua_getfenv (L, 1);
  return lua_gobject_marshal_field (L, record->addr, getmode, 1, 2, 3);
}
...
record_cast (lua_State *L)
{
  ...
  lua_gobject_record_2lua (L, record->addr, FALSE, 1);
  return 1;
}
...
record_fromarray (lua_State *L)
{
  ...
  lua_getfenv (L, 1);
  lua_gobject_record_2lua (L, ((guint8 *) record->addr) + size * index, own, parent);
  return 1;
}
...
void
lua_gobject_record_init (lua_State *L)
{
  ...
  /* Create caches. */
  lua_gobject_cache_create (L, &record_cache, "v");
  lua_gobject_cache_create (L, &parent_cache, "k");
