...
GIRepository *
lua_gobject_gi_get_repository (void)
{
...
int
lua_gobject_gi_info_new (lua_State *L, GIBaseInfo *info)
{
  if (info)
    {
      GIBaseInfo **ud_info;
      ...
      *ud_info = info;
      luaL_getmetatable (L, LUA_GOBJECT_GI_INFO);
...
gpointer
lua_gobject_gi_load_function (lua_State *L, int typetable, const char *name)
{
  GIBaseInfo **info;
  ...
  info = lua_gobject_udata_test (L, -1, LUA_GOBJECT_GI_INFO);
...
#define LUA_GOBJECT_GI_INFOS "lua_gobject.gi.infos"

static int
infos_len (lua_State *L)
{
  Infos* infos = luaL_checkudata (L, 1, LUA_GOBJECT_GI_INFOS);
  ...
  if (lua_type (L, 2) == LUA_TNUMBER)
    {
      ...
      return lua_gobject_gi_info_new (L, infos->item_get (infos->info, n));
    }
  else
    {
...
infos_new (lua_State *L, GIBaseInfo *info, gint count, InfosItemGet item_get)
{
  ...
  luaL_getmetatable (L, LUA_GOBJECT_GI_INFOS);
  lua_setmetatable (L, -2);
...
#undef H
  ...
  if (strcmp (prop, "fullname") == 0)
    {
      lua_concat (L, lua_gobject_type_get_name (L, *info));
      return 1;
    }
  ...
  if (strcmp (prop, "deprecated") == 0)
	...
	gi_base_info_ref (container);
      return lua_gobject_gi_info_new (L, container);
    }
	...
	ti = gi_field_info_get_type_info (GI_FIELD_INFO (*info));
      ...
      if (ti)
	return lua_gobject_gi_info_new (L, GI_BASE_INFO (ti));
    }
  ...
  if (GI_IS_REGISTERED_TYPE_INFO (*info))
      ...
      else if (GI_IS_INTERFACE_INFO (*info))
	{
	  if (strcmp (prop, "type_struct") == 0)
	    return
	      lua_gobject_gi_info_new (L, GI_BASE_INFO (gi_interface_info_get_iface_struct (GI_INTERFACE_INFO (*info))));
      ...
      else if (GI_IS_OBJECT_INFO (*info))
	{
	  if (strcmp (prop, "parent") == 0)
	    return lua_gobject_gi_info_new (L, GI_BASE_INFO (gi_object_info_get_parent (GI_OBJECT_INFO (*info))));
	  else if (strcmp (prop, "type_struct") == 0)
	    return lua_gobject_gi_info_new (L, GI_BASE_INFO (gi_object_info_get_class_struct (GI_OBJECT_INFO (*info))));
  ...
  if (GI_IS_CALLABLE_INFO (*info))
	...
	return lua_gobject_gi_info_new (L, GI_BASE_INFO (gi_callable_info_get_return_type (GI_CALLABLE_INFO (*info))));
  ...
  if (GI_IS_TYPE_INFO (*info))
      ...
      else if (strcmp (prop, "params") == 0)
	{
	  if (tag == GI_TYPE_TAG_ARRAY || tag == GI_TYPE_TAG_GLIST ||
	    ...
	    {
	      lua_newtable (L);
	      lua_gobject_gi_info_new (L, GI_BASE_INFO (gi_type_info_get_param_type (GI_TYPE_INFO (*info), 0)));
	      lua_rawseti (L, -2, 1);
	      if (tag == GI_TYPE_TAG_GHASH)
		{
		  lua_gobject_gi_info_new (L, GI_BASE_INFO (gi_type_info_get_param_type (GI_TYPE_INFO (*info), 1)));
		  lua_rawseti (L, -2, 2);
		}
	      return 1;
	    }
	...
	{
	  lua_gobject_gi_info_new (L, gi_type_info_get_interface (GI_TYPE_INFO (*info)));
	  return 1;
	}
...
info_eq (lua_State *L)
{
  GIBaseInfo **i1 = luaL_checkudata (L, 1, LUA_GOBJECT_GI_INFO);
  GIBaseInfo **i2 = luaL_checkudata (L, 2, LUA_GOBJECT_GI_INFO);
...
#define LUA_GOBJECT_GI_RESOLVER "lua_gobject.gi.resolver"

static int
resolver_index (lua_State *L)
{
  gpointer address;
  GITypelib **typelib = luaL_checkudata (L, 1, LUA_GOBJECT_GI_RESOLVER);
...
#define LUA_GOBJECT_GI_NAMESPACE "lua_gobject.gi.namespace"

static int
namespace_len (lua_State *L)
{
  const gchar *ns = luaL_checkudata (L, 1, LUA_GOBJECT_GI_NAMESPACE);
  lua_pushinteger (L, gi_repository_get_n_infos (lua_gobject_gi_get_repository (), ns));
  return 1;
}
...
namespace_index (lua_State *L)
{
  ...
  if (lua_type (L, 2) == LUA_TNUMBER)
    {
      GIBaseInfo *info = gi_repository_get_info (lua_gobject_gi_get_repository (), ns,
						 lua_tointeger (L, 2) - 1);
      return lua_gobject_gi_info_new (L, info);
    }
  ...
  if (strcmp (prop, "dependencies") == 0)
    {
      gchar **deps = gi_repository_get_dependencies (lua_gobject_gi_get_repository (), ns, NULL);
      if (deps == NULL)
	lua_pushnil (L);
      else
	{
    ...
    {
      lua_pushstring (L, gi_repository_get_version (lua_gobject_gi_get_repository (), ns));
      return 1;
    }
  ...
  else if (strcmp (prop, "resolve") == 0)
    {
      ...
      lua_setmetatable (L, -2);
      *udata = gi_repository_require (lua_gobject_gi_get_repository (), ns, NULL, 0, NULL);
      return 1;
    }
  else
    /* Try to lookup the symbol. */
    return lua_gobject_gi_info_new (L, gi_repository_find_by_name (lua_gobject_gi_get_repository (), ns, prop));
}
...
namespace_new (lua_State *L, const gchar *namespace)
{
  ...
  luaL_getmetatable (L, LUA_GOBJECT_GI_NAMESPACE);
  lua_setmetatable (L, -2);
  strcpy (ns, namespace);
  return 1;
}
...
gi_require (lua_State *L)
{
  ...
  if (typelib_dir == NULL)
    typelib = gi_repository_require (lua_gobject_gi_get_repository (), namespace, version, 0, &err);
  else
    typelib = gi_repository_require_private (lua_gobject_gi_get_repository (), typelib_dir, namespace,
					     version, 0, &err);
  if (!typelib)
    {
      lua_pushboolean (L, 0);
...
gi_index (lua_State *L)
{
  if (lua_type (L, 2) == LUA_TLIGHTUSERDATA)
	...
	? gi_repository_find_by_gtype (lua_gobject_gi_get_repository (), gtype) : NULL;
      return lua_gobject_gi_info_new (L, info);
    }
  else if (lua_type (L, 2) == LUA_TNUMBER)
    {
      GQuark domain = (GQuark) lua_tointeger (L, 2);
      GIBaseInfo *info = GI_BASE_INFO (gi_repository_find_by_error_domain (lua_gobject_gi_get_repository (), domain));
      return lua_gobject_gi_info_new (L, info);
    }
  else
    {
      const gchar *ns = luaL_checkstring (L, 2);
      if (gi_repository_is_registered (lua_gobject_gi_get_repository (), ns, NULL))
...
static const Reg gi_reg[] = {
  { LUA_GOBJECT_GI_INFOS, gi_infos_reg },
  { LUA_GOBJECT_GI_INFO, gi_info_reg },
  { LUA_GOBJECT_GI_NAMESPACE, gi_namespace_reg },
  { LUA_GOBJECT_GI_RESOLVER, gi_resolver_reg },
  { NULL, NULL }
};
...
void
lua_gobject_gi_init (lua_State *L)
{
  const Reg *reg;
...
gsize
lua_gobject_struct_info_get_size (GIStructInfo *info)
{
  if (parameter_info == NULL)
    parameter_info = gi_repository_find_by_name (lua_gobject_gi_get_repository (), "GObject", "Parameter");
...
gint
lua_gobject_field_info_get_offset (GIFieldInfo *info)
{
  if (parameter_value_info == NULL)
    {
      if (parameter_info == NULL)
	parameter_info = gi_repository_find_by_name (lua_gobject_gi_get_repository (),
						     "GObject", "Parameter");
