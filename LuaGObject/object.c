  ...
  for (; gtype != G_TYPE_INVALID; gtype = g_type_parent (gtype))
    {
      ...
      lua_gobject_type_get_repotype (L, gtype, NULL);
      if (!lua_isnil (L, -1))
	break;
...
object_type_error (lua_State *L, int narg, GType gtype)
{
  ...
  if (found_gtype != G_TYPE_INVALID)
	...
	lua_pushliteral (L, "lua_gobject.object");
      else
...
gpointer
lua_gobject_object_get_function_ptr (GIObjectInfo *info,
...
object_load_function (lua_State *L, GType gtype, const gchar *name)
{
  ...
  if (object_type (L, gtype) != G_TYPE_INVALID)
    {
      func = lua_gobject_gi_load_function (L, -1, name);
      lua_pop (L, 1);
    }
  return func;
}
...
object_refsink (lua_State *L, gpointer obj, gboolean no_sink)
{
  ...
  /* Check whether object has registered fundamental 'ref'
     function. */
  GIObjectInfo *info = GI_OBJECT_INFO (gi_repository_find_by_gtype (lua_gobject_gi_get_repository (), gtype));
  if (info == NULL)
    info = GI_OBJECT_INFO (gi_repository_find_by_gtype (lua_gobject_gi_get_repository (), G_TYPE_FUNDAMENTAL (gtype)));
  if (info != NULL && gi_object_info_get_fundamental (info))
    {
      GIObjectInfoRefFunction ref =
	lua_gobject_object_get_function_ptr (info, gi_object_info_get_ref_function_name);
      gi_base_info_unref (info);
      if (ref != NULL)
	{
	  ref (obj);
...
object_unref (lua_State *L, gpointer obj)
{
  ...
  if (info != NULL && gi_object_info_get_fundamental (info))
	...
	lua_gobject_object_get_function_ptr (info, gi_object_info_get_unref_function_name);
      gi_base_info_unref (info);
      if (unref != NULL)
	{
	  unref (obj);
...
object_tostring (lua_State *L)
{
  ...
  lua_pushfstring (L, "lua_gobject.obj %p:%s(%s)", obj, lua_tostring (L, -1),
		   g_type_name (gtype));
  return 1;
}

gpointer
lua_gobject_object_2c (lua_State *L, int narg, GType gtype, gboolean optional,
...
int
lua_gobject_object_2lua (lua_State *L, gpointer obj, gboolean own, gboolean no_sink)
{
...
object_access (lua_State *L)
{
  ...
  object_get (L, 1);
  lua_getfenv (L, 1);
  return lua_gobject_marshal_access (L, getmode, 1, 2, 3);
}
...
object_field (lua_State *L)
{
  ...
  lua_getfenv (L, 1);
  return lua_gobject_marshal_field (L, object, getmode, 1, 2, 3);
}
...
object_new (lua_State *L)
{
  if (lua_islightuserdata (L, 1))
    ...
    return lua_gobject_object_2lua (L, lua_touserdata (L, 1), lua_toboolean (L, 2),
			    lua_toboolean (L, 3));
  else
    {
	 ...
	 not export this method in the typelib. */
      ...
      GType gtype = lua_gobject_type_get_gtype (L, 1);
      luaL_checktype (L, 2, LUA_TTABLE);

      /* Find BaseInfo of GParameter. */
      gparam_info = gi_repository_find_by_name (lua_gobject_gi_get_repository (), "GObject", "Parameter");
      *lua_gobject_guard_create (L, (GDestroyNotify) gi_base_info_unref) = gparam_info;
      ...
      for (i = 0; i < size; ++i)
	{
	  ...
	  lua_gettable (L, 2);
	  lua_gobject_type_get_repotype (L, G_TYPE_INVALID, gparam_info);
	  lua_gobject_record_2c (L, -2, &params[i], TRUE, FALSE, FALSE, FALSE);
	  lua_pop (L, 1);
	}

      /* Create the object and return it. */
      return lua_gobject_object_2lua (L, g_object_newv (gtype, size, params),
			      TRUE, FALSE);
...
void
lua_gobject_object_init (lua_State *L)
{
  char *id;
  ...
  lua_gobject_cache_create (L, &cache, "v");
  ...
  id = g_strdup_printf ("lua_gobject:%p", L);
